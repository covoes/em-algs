function [ bestPartition EMStepsRet ] = GA_EM( data, R , pMutation, pRecombination,...
    populationSize, correlationThreshold, maxClusters, annihilationThreshold, kmeansInitialization)
...
while(c_end ~= 5)
    ...
    %a_min is the best solution found so far, updated in the return of perform_EM
    if(sum(a_min.bits)~=OldSize)
        c_end = 0;
        OldSize = sum(a_min.bits);
    else
        c_end = c_end + 1;
    end
